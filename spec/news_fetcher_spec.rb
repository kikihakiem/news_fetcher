...
describe NewsFetcher do
  ...
  describe 'each_news' do
    ...
    subject do
      results = []
      NewsFetcher.each_news(feed_url) do |filename, xml|
        results << {filename: filename, xml: xml}
      end

      results
    end

    it 'yields filename and its content' do
      subject.first[:filename].must_equal 'file1'
      subject.first[:xml].must_equal 'file1 content'
      subject.last[:filename].must_equal 'file2'
